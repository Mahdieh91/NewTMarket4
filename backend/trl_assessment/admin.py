# trl_assessment/admin.py
import json
from django.contrib import admin
from django.utils.safestring import mark_safe
from django.utils.html import format_html
from .models import TRLAssessment
from .trl_logic import TRL_QUESTIONS
...
class TRLAssessmentAdmin(admin.ModelAdmin):
    ...
    list_filter = ('trl', 'status', 'created_at')
    ...
    def answers_pretty(self, obj):
        ...
        # مرتب‌سازی بر اساس شناسه سوال (برای خوانایی بهتر)
    ...
    def progress_bar(self, obj):
        """
        نوار پیشرفت نشان‌دهنده درصد پاسخ‌های 'yes' نسبت به کل سوالات TRL
        (مجموع سوالات تمام سطوح ۱ تا ۹)
        """
        if not obj.answers:
            return "بدون پاسخ"

        # محاسبه تعداد کل سوالات TRL از دیکشنری TRL_QUESTIONS
        total_questions = sum(len(qids) for qids in TRL_QUESTIONS.values())
        yes_count = sum(1 for v in obj.answers.values() if v.get('value') == 'yes')
    ...
    def level_breakdown(self, obj):
        """
        نمایش وضعیت هر سطح (۱ تا ۹) با رنگ‌های سبز/قرمز/زرد/خاکستری
        """
        if not obj.answers:
        ...
        for level in range(1, 10):  # ۱ تا ۹
            qids = TRL_QUESTIONS.get(level, [])
            if not qids:
                continue
        ...
        html += '</div></div>'
        return mark_safe(html)
    level_breakdown.short_description = "وضعیت سطح‌بندی TRL"
