...
// ===== تابع محاسبه MRL =====
function calculateMRL(answers: Answer[]): AssessmentResultType {
  ...
  const achievedLevels = achieved.map((lvl) => ({ level: lvl, description: `سطح ${lvl} MRL` }));
...
export default function MRLPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const level1Questions = MRL_QUESTIONS[1]?.questions.map(q => q.id) || [];
    const answeredIds = answers.map(a => a.questionId);
    const missingLevel1 = level1Questions.filter(id => !answeredIds.includes(id));
    if (missingLevel1.length > 0) {
      showToast('لطفاً به همه سوالات سطح ۱ (MRL 1) پاسخ دهید.', 'error');
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/mrl/assess/`, {
        method: 'POST',
    ...
    <div className="trl-page-container" dir="rtl" ref={topRef}>
      ...
      <div className="trl-card">
        <div className="trl-header">
          <h1 className="trl-title">ارزیابی سطح آمادگی تولید</h1>
          <p className="trl-subtitle">Manufacturing Readiness Level (MRL)</p>
          ...
          {!isAuthenticated && (
            <div className="trl-auth-warning">
              ...
              <button
                onClick={() => router.push(`/login?next=/mrl?return=${encodeURIComponent(returnUrl)}`)}
        ...
        <div className="trl-intro">
          <div className="trl-intro-box">
            ...
            <div>
              ...
              <p>سطح آمادگی تولید (MRL) به‌عنوان یکی از معیارهای معتبر بین‌المللی، میزان بلوغ فرآیندهای تولید یک فناوری را از مرحله‌ی شناسایی مفاهیم اولیه تا تولید انبوه و پایدار مورد سنجش قرار می‌دهد.</p>
              <p><strong>این ارزیابی صرفاً بر مبنای وضعیت عینی، مستند و فعلی فرآیندهای تولید قابل پذیرش است. هرگونه ادعای غیرمستند، برآوردی یا مبتنی بر برنامه‌های آتی، مبنای ارزیابی قرار نخواهد گرفت.</strong></p>
        ...
        <form onSubmit={handleSubmit} className="trl-form">
          {Object.entries(MRL_QUESTIONS).map(([level, levelData]) => {
