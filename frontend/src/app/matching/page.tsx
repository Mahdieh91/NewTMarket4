...
const normalizeScore = (
   ...
   * match_percentage = 94
   *
   * This helper is only defensive and does not modify
   * the actual API value.
   */
...
const getRiskLevel = (
  ...
  /*
   * Risk is not a field in MatchResult.
   * Therefore it is derived only from the actual
   * match percentage and is clearly presented as
   * an indicative risk level.
   */
  if (percentage >= 85) {
    return 'low';
  }
...
export default function MatchingPage() {
  ...
  const loadMatches = useCallback(
    async () => {
      ...
      try {
        ...
        /*
         * فقط داده‌هایی که ساختار واقعی MatchResult
         * را دارند وارد state می‌شوند.
         */
        const validResults =
          resultList.filter(
            (item) =>
        ...
        /*
         * Need ID واقعی از MatchResult گرفته می‌شود.
         * برای جلوگیری از حدس، فقط شناسه Need را نگه
         * می‌داریم و اطلاعاتی که Backend Matching فعلی
         * ارسال نمی‌کند جعل نمی‌کنیم.
         */
        const firstNeed =
          validResults[0]?.need;
        ...
        if (
          ...
          setNeedSummary({
            id: Number(firstNeed),
            title: `نیاز شماره ${new Intl.NumberFormat(
              'fa-IR'
            ).format(Number(firstNeed))}`,
            description:
              'اطلاعات کامل نیاز از Endpoint فعلی Matching ارسال نشده است.',
            industry: '',
            budget: '',
            timeline: '',
            status: '',
    ...
    useMemo(() => {
      return [...filteredMatches].sort(
        (a, b) => {
          ...
          /*
           * MatchResult فعلی هیچ providerRating
           * ندارد. بنابراین در حالت rating
           * از score واقعی استفاده می‌کنیم و
           * امتیاز جعلی تولید نمی‌کنیم.
           */
          return (
            Number(b.score) -
            Number(a.score)
  ...
  const startNegotiation =
    async (
      ...
      try {
        /*
         * Backend فعلی NegotiationViewSet
         * فیلد product را دریافت می‌کند.
         *
         * Frontend هیچ supply/supplyId ارسال نمی‌کند.
         */
        const response =
          await authenticatedFetch(
        ...
        if (
          ...
          alert(
            data?.detail ||
              data?.error ||
              'امکان شروع مذاکره برای این محصول وجود ندارد.'
          );
        ...
        if (
          ...
          alert(
            data?.detail ||
              data?.error ||
              'محصول موردنظر در Backend پیدا نشد.'
          );
        ...
        if (
          ...
          alert(
            'مسیر ایجاد مذاکره در Backend اجازه POST نمی‌دهد.'
          );
        ...
        if (
          ...
          console.error(
            '❌ پاسخ مذاکره فاقد شناسه مذاکره است:',
            data
          );
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">
                راهکارهای متناسب با نیازهای ثبت‌شده شما
              </p>
            </div>
        ...
        {isLoading ? (
          <div className="rounded-2xl border border-slate-200 bg-white p-12 shadow-sm">
            <div className="flex flex-col items-center justify-center">
              <div className="w-12 h-12 rounded-full border-4 border-slate-200 border-t-[#14B8A6] animate-spin mb-4" />

              <p className="text-sm font-bold text-slate-700">
                در حال دریافت نتایج تطبیق...
              </p>

              <p className="text-xs text-slate-500 mt-1">
                نتایج واقعی از سامانه دریافت می‌شوند.
              </p>
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {needSummary ? (
                  <div className="space-y-3">
                    ...
                    <p className="text-xs text-slate-500 leading-relaxed">
                      اطلاعات تفصیلی نیاز در Endpoint فعلی
                      Matching ارسال نشده است. نتایج زیر مستقیماً
                      از MatchResultهای واقعی مربوط به این نیاز
                      دریافت شده‌اند.
                    </p>
                  </div>
                ) : (
                  <div className="text-center py-5">
                    ...
                    <p className="text-xs text-slate-500">
                      هنوز نتیجه تطبیقی برای نیاز شما ثبت نشده است.
                    </p>
                  </div>
                )}
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              {sortedMatches.length ===
                0 && (
                <div className="rounded-2xl border border-slate-200 bg-white text-center py-16 px-6">
                  <Search
                    size={48}
                  ...
                  <p className="text-sm text-slate-500">
                    در حال حاضر نتیجه تطبیقی مطابق
                  ...
                  <button
                    ...
                    className="mt-5 px-4 py-2 rounded-xl border border-[#1E3A8A] text-[#1E3A8A] text-sm font-bold hover:bg-[#1E3A8A]/5 transition"
                  >
                    حذف فیلترها
                  </button>
                </div>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-slate-600 leading-relaxed mb-3">
                              {match.product_description ||
                                'توضیحات محصول برای این راهکار ثبت نشده است.'}
                            </p>
